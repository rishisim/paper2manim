...
def run_segmented_pipeline(
    ...
    def _run_coder_for_segment(seg: dict) -> dict:
        seg_id = seg["id"]
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        ...
        while pending:
            # Stream intermediate status updates while workers are still running.
            while True:
                try:
                    partial = status_queue.get_nowait()
                except Empty:
                    break
            ...
            finished_now = [f for f in list(pending) if f.done()]
            for fut in finished_now:
                ...
                yield {
                    "stage": "code",
                    "segment_id": seg_id,
                    "segment_phase": "done" if has_video else "failed",
                    "segment_final": True,
                    "status": (
        ...
        # Drain any remaining queue entries after all workers complete.
        while True:
            try:
                partial = status_queue.get_nowait()
            except Empty:
